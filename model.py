...
class SkincareModel:
    ...
    [Требования]
    - Строго используй указанный метод "{recommendation['method']}" и тип процедуры "{recommendation['type']}" в тексте. Не заменяй их другими методами или процедурами.
    - Следуй формату:
      Инструкция: [1-2 предложения, начинающиеся с глагола]
      Противопоказания: [3 пункта, разделённые запятыми]
      Ожидаемые результаты: [2-3 эффекта, разделённые запятыми]
    ...
    def _parse_generated_text(self, text, method, type_, context):
        ...
        # Ищем "Инструкция", "Противопоказания", "Ожидаемые результаты" с учётом возможных пробелов и двоеточий
        ...
        if self.classifier and self.regressor and self.label_encoders and self.vectorizer:
            try:
                for prob in problems:
                    # Убедимся, что symptoms — это список строк
                    symptoms_list = symptoms if isinstance(symptoms, list) else [symptoms]
                    input_df = pd.DataFrame([{
                        'problem': prob,
                        'skin_type': skin_type,
                        'age_range': age_range,
                        'symptoms': symptoms_list  # Передаём как список строк
                    }])
